...
function process_gpio() {
  ...
  if [ "$(gpioget $(gpiofind "${LINE}") 2>/dev/null||echo 1)" -eq 0 ]; then
    systemctl --no-block start "dbus-object-present@${ITEM}:Present.service"
  else
    systemctl --no-block start "dbus-object-present@${ITEM}:Removed.service"
  fi
  eval echo ${TMPL} | tr -d '\n' >> ${CFG_FILE};
}
...
do
  process_gpio "PLUG_DETECT_DIMM_${i^^}" "system-chassis-motherboard-dimm_${i}"
done

for i in {1..9}
do
  process_gpio "RSSD0${i}_PRESENT_N" "system-chassis-motherboard-rssd_0${i}"
done

for i in {10..32}
do
  process_gpio "RSSD${i}_PRESENT_N" "system-chassis-motherboard-rssd_${i}"
done
